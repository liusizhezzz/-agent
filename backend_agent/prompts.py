...
def compile_prompt(agent_id: str, *, user_text: str = "", rag: str = "", mode: str = "chat") -> str:
    profile = get_profile(agent_id)
    mode = mode if mode in {"chat", "sleep_support"} else "chat"
    mode_rule = ("当前回合模式：普通闲聊。只围绕用户刚说的内容回应，不主动谈睡眠、躺下、放松、呼吸或CBT-I。" if mode == "chat" else "当前回合模式：睡眠支持。仅根据用户明确表达的睡眠困扰，谨慎使用相关CBT-I方法。")
    parts = [BASE_SAFETY, mode_rule, f"你的身份是{profile['name']}，世界物件是“{profile['worldObject']}”。", f"人格：{profile['personality']}", f"CBT-I侧重点：{'、'.join(profile['cbtiFocus'])}", f"适用场景：{'、'.join(profile['适用场景'])}", f"说话方式：{profile['speechStyle']}", f"跟进规则：{'；'.join(profile['followUpRules'])}", f"静默规则：{profile['silencePolicy']}", f"告别规则：{profile['farewellRules']}", f"代表性话术（仅作身份背景，不要机械复述）：{profile['representativeLine']}", "自由对话优先：除非用户明确需要，否则不要主动转入CBT-I或提出练习；允许自然闲聊、接话、讲故事和表达观点。"]
